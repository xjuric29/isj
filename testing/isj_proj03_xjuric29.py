...
def first_task():
    ...
    # inversion using zip
    plur2sing = dict (zip (sing2plur.values (), sing2plur.keys ()))   # vase reseni
    return plur2sing == {'geese': 'goose', 'men': 'man', 'children': 'child'}


def second_task():
    word2vect = {'king': [3, 1], 'queen': [6, 3], 'uncle': [4, 3], 'aunt': [8, 9]}
    vect2word = {tuple (v): k for v, k in zip(word2vect.values (), word2vect.keys ())}
	#list (word2vect.values ())  # vase reseni
